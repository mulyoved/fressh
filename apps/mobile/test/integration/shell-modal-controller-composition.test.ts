...
void test('shell detail composes focused modal controllers without shell-modals', () => {
	...
	const skillBlock = extractBlock(
		...
		'const { openConfigDialog, configureCommands }',
	);
	...
	assert.match(browserBlock, /workmux: ports\.workmux/);
	...
	assert.match(skillBlock, /workmux: ports\.workmux/);
	...
	assert.match(source, /openSkillSelector,/);
	assert.match(source, /openWisprTextEditor: wispr\.openTextEditor/);
	assert.doesNotMatch(
		source,
...
void test('shell detail tracks tmux-only lifecycle changes separately from target identity', () => {
	...
	assert.match(source, /remoteTarget:\s*\{[\s\S]*?hostCommands: connection,/);
	assert.doesNotMatch(
		source,
